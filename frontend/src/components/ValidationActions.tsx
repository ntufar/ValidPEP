  ...
  const fetchHtmlReport = async (result: ValidateResponse): Promise<string> => {
    ...
    return `
      ...
      <head>
          ...
          <title>Validation Report - ${result.id || 'N/A'}</title>
          <style>
      ...
      <body>
          <div class="container">
              ...
              ${result.issues.filter(issue => issue.severity === 'info').length > 0 ? `
              <div class="section">
                  <h2>Information (${result.issues.filter(issue => issue.severity === 'info').length})</h2>
                  ${result.issues.filter(issue => issue.severity === 'info').map(issueToHtml).join('')}
              </div>` : ''}
          </div>
      </body>
      </html>
