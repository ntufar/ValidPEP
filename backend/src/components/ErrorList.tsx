...
import type { Issue, IssueSeverityType } from '../types/validation';
...
const ErrorList: React.FC<ErrorListProps> = ({ issues, onIssueClick }) => {
  ...
  const getSeverityColor = (severity: IssueSeverity | IssueSeverityType) => {
    const severityValue = typeof severity === 'string' ? severity : String(severity);
